...
import type { AstPath } from '../types/index';
...
const es5 = {
  ...
  MemberExpression(astPath: AstPath<ESTree.MemberExpression>) {
    ...
    const prop = computed ? this.visitNode(property, scope) : (<ESTree.Identifier>property).name;
  ...
  BlockStatement(astPath: AstPath<ESTree.BlockStatement>) {
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      if (Signal.isBreak(res) || Signal.isContinue(res) || Signal.isReturn(res)) {
        return res;
      }
    }
  },
  ...
  ReturnStatement(astPath: AstPath<ESTree.ReturnStatement>) {
    const { node, scope } = astPath;
    return new Signal(SignalType.return, node.argument ? this.visitNode(node.argument, scope) : undefined);
  },
  IfStatement(astPath: AstPath<ESTree.IfStatement>) {
    ...
    return alternate ? this.visitNode(alternate, scope) : undefined;
  },
};
export default es5;
